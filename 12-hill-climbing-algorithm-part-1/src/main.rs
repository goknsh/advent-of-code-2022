    ...
    if args.len() == 2 {
        match fs::read_to_string(&args[1]) {
            Ok(input) => {
                ...
                let mut queue = vec![QueueNode { distance: 0, location: start }];
                ...
                println!("Fewest steps required to move from your current position to the location that should get the best signal: {}", final_distance)
            }
